...
## 2026-05-18 - [MEDIUM] Unbounded Date String Parsing DoS
**Vulnerability:** Zod validation schemas (`createTaskSchema`) using `z.string().transform((val) => new Date(val))` lacked a `.max()` constraint on the string input. Attackers could send an excessively long string, which Zod would attempt to parse and potentially the JavaScript engine would attempt to convert to a Date, leading to a Denial of Service (DoS) via CPU exhaustion.
**Learning:** When transforming strings into objects like Dates, it's critical to limit the length of the string before the transformation occurs. Zod executes string validations (like `.max()`) before `.transform()` functions.
**Prevention:** Always prepend a strict `.max()` length constraint (e.g., `.max(100)`) to the string validation before `.transform()` when converting strings to Dates or other complex objects to prevent DoS attacks.
## 2026-05-18 - [MEDIUM] Fix runtime crash on empty task update payload
**Vulnerability:** Empty update payloads passed directly to Drizzle ORM's `.set()` method (after relational data has been destructured out) can cause an application crash or unhandled runtime error ("No values to set"). This opens up a potential Denial of Service (DoS) attack if invoked repeatedly with an empty payload.
**Learning:** We always need to ensure that the constructed object payload passed into `db.update().set()` has at least one key before executing the database operation. Destructuring `taskData` and stripping relational data often leaves an empty object when the user intended only to update subtasks.
**Prevention:** Always wrap `db.update().set(data)` executions within an `if (Object.keys(data).length > 0)` check and define an explicit fallback to gracefully skip execution and prevent Drizzle from throwing a query builder error.
## 2026-05-18 - [MEDIUM] Server Action Payload Empty `.set()` Crash
**Vulnerability:** Server Actions in Next.js using Drizzle ORM (`updateTask`) were stripping out relational fields (like `subtasks`, `labels`) from validated payloads using dynamic destructuring (`const { subtasks, labels, ...taskData } = validatedData;`) and blindly passing the result (`taskData`) to `db.update().set()`. If a payload contained *only* relational fields (and no actual table columns), `taskData` would be an empty object. Passing an empty object to Drizzle's `.set()` method throws a runtime error ("No values to set"), crashing the Server Action.
**Learning:** Destructuring payloads to prevent mass assignment is correct, but it introduces a new edge case where the resulting object to update might be entirely empty. Drizzle ORM expects at least one column to update in a `.set()` call.
**Prevention:** Always check if the dynamically destructured object has properties (e.g., `if (Object.keys(taskData).length > 0)`) before executing a `db.update().set()` operation, and fallback gracefully if it is empty.
## 2026-05-18 - [MEDIUM] Missing Input Validation in API Route (Mass Assignment)
**Vulnerability:** API routes and Server Actions in Next.js (`src/app/api/lists/[id]/route.ts` and `src/app/actions/task.ts`) were passing validated update payload data directly to database `update().set()` without verifying if the update payload was completely empty.
**Learning:** If a client sends an empty payload or a payload without any of the updated properties present (like `name`, `color`, `emoji` in `src/app/api/lists/[id]/route.ts`), filtering out the `undefined` values results in an empty object. Passing an empty object to Drizzle ORM's `.set()` throws an unhandled `Error: No values to set` exception which results in a 500 internal server error or a server crash.
**Prevention:** Always verify that an update object is not empty (`Object.keys(updateData).length > 0`) before executing `db.update().set()`. If it is empty, gracefully skip the update operation and return the existing data or fall back appropriately instead of allowing the query builder to crash.
## 2026-05-18 - [HIGH] Next.js Vulnerability Audit Upgrade
**Vulnerability:** The project's Next.js version (16.2.3) was subject to multiple high and moderate severity vulnerabilities as flagged by `pnpm audit`, including Middleware bypass in App Router segment-prefetch, SSR Cross-Site Scripting (XSS), and cache poisoning vectors.
**Learning:** Outdated framework versions can harbor critical vulnerabilities even if the project code itself adheres to strict standards. Overrides in package.json using `pnpm.overrides` provide a way to forcefully resolve these deep dependencies before a major framework release.
**Prevention:** Regularly run `pnpm audit` and explicitly bump Next.js framework versions across the repository when security advisories are issued, verifying builds and applying overrides for nested dependencies if necessary.
## 2026-05-20 - [HIGH] Global DoS via Right-most x-forwarded-for IP Extraction
**Vulnerability:** When implementing IP-based rate limiting on the `/api/lists` and `/api/tasks` endpoints, the `x-forwarded-for` header was parsed by extracting the right-most IP address (`.split(',').pop()`). The `x-forwarded-for` header format is `<client-ip>, <proxy1-ip>, <proxy2-ip>`. By extracting the right-most IP, the rate limiter inadvertently targeted the proxy server closest to the application (e.g., a CDN or internal load balancer). This resulted in all traffic routed through that proxy sharing a single rate-limit bucket, causing an immediate, application-wide Denial of Service (DoS) for all legitimate users.
**Learning:** While the left-most IP (`.split(',')[0]`) in `x-forwarded-for` is spoofable by attackers passing their own header value, it represents the original client. The right-most IP (`.pop()`) represents the infrastructure proxy. In unconfigured or basic environments, using `[0]` safely prevents global DoS at the cost of potential individual spoofing bypasses, whereas `.pop()` is highly destructive.
**Prevention:** Always extract the left-most IP (`.split(',')[0]?.trim()`) when using `x-forwarded-for` for basic rate limiting unless a strict, trusted proxy topology array is explicitly configured and validated in the application layer.
## 2026-05-24 - [MEDIUM] Unbounded Date String Parsing DoS in API Schemas
**Vulnerability:** Zod validation schemas (`createTaskSchema` and `updateTaskSchema`) were using `z.date()` which rejected valid JSON API date strings. If naively fixed by coercing the string to a date without bounds, an attacker could send an excessively long string. Zod would attempt to parse this, and the JavaScript engine would attempt to convert it to a Date, leading to a Denial of Service (DoS) via CPU exhaustion.
**Learning:** When transforming strings into objects like Dates in validation schemas, it's critical to limit the length of the string *before* the transformation occurs. Zod executes string validations (like `.max()`) before `.transform()` functions. Also, standard `z.date()` strictly expects a native JavaScript Date object, which fails on typical `application/json` payloads where dates are serialized as ISO strings.
**Prevention:** Always use a safely bounded parsing union for dates in JSON APIs: `z.union([z.string().max(100).transform((val) => val ? new Date(val) : undefined).refine((d) => !d || !isNaN(d.getTime()), 'Invalid date'), z.date()])`.
