...
## 2024-05-24 - Combine queries using leftJoin to reduce DB calls
**Learning:** When fetching a parent entity and its children (e.g., a List and its Tasks) using Drizzle's core Query Builder (to avoid relational API microtask overhead), executing two separate queries is less efficient. You can combine them into a single DB call using `.leftJoin()` and processing the flat result rows, effectively reducing the number of database roundtrips.
**Action:** When converting relational queries to core query builder, use `.leftJoin()` to combine fetching parent and child records into a single database call, then group the results in memory.
...
\n## 2026-07-02 - Hoist static useHotkeys configuration\n**Learning:** When using `useHotkeys` from `react-hotkeys-hook`, passing an inline object literal for options (e.g., `{ enableOnFormTags: false }`) inside a functional component causes the options reference to change on every render. This triggers unnecessary teardown and recreation of the global keyboard event listeners within the hook's internal `useEffect`.\n**Action:** Always hoist static configuration objects for `useHotkeys` options outside the component definition to maintain reference equality across renders, preventing unnecessary DOM event manipulation and garbage collection overhead.
## 2026-07-03 - Cache repeated Date object allocations in renders
**Learning:** Performing `new Date()` multiple times for the same value within a React component's render function (e.g., inside template literals, JSX attributes, and conditional logic) creates excessive, short-lived objects that trigger frequent garbage collection, especially in lists.
**Action:** Precompute and cache derived values and `Date` objects once at the top of the component render body, then reuse those cached variables throughout the JSX to eliminate redundant object allocations.
