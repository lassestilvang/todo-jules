...
**Action:** Always wrap complex list item components (e.g. `TaskComponent`) in `React.memo()` when they are part of a drag-and-drop reorder operation. This prevents an O(N) re-render of untouched list items, avoiding massive CPU overhead and janky animations in large lists.
