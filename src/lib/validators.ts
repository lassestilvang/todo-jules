...
export const createTaskSchema = z.object({
  ...
  date: z.union([z.string().max(100, 'Date string is too long').transform((val) => val ? new Date(val) : undefined), z.date()]).optional().nullable(),
  deadline: z.union([z.string().max(100, 'Date string is too long').transform((val) => val ? new Date(val) : undefined), z.date()]).optional().nullable(),
...
export const updateListSchema = z.object({
  ...
  emoji: z.string().min(1, 'Emoji is required').max(20).optional(),
});
